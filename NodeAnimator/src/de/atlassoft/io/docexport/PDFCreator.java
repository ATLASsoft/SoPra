...
class PDFCreator {
	 ...
	 *    
	 */
	protected void addTitlePage (Document document, String title, String descrption) throws DocumentException {
		if (document == null) {
			throw new IllegalArgumentException(
					"document must not be null");
		}
	 ...
	 *    
	 */
	protected void addScheduleContent (Document document, ScheduleScheme schedule) throws DocumentException {
		if (document == null) {
			throw new IllegalArgumentException(
					"document must not be null");
		}
		
		Paragraph scheduleName = new Paragraph("Fahrplanname: " + schedule.getID());
		Paragraph scheduleType = new Paragraph("Fahrplantyp: " + schedule.getScheduleType());
		Paragraph firstDay = new Paragraph("Erster Fahrtag : " + getWeekDay(schedule.getFirstRide().DAY_OF_WEEK) + ",der " + schedule.getFirstRide().DAY_OF_MONTH + "." + schedule.getFirstRide().MONTH + "." + schedule.getFirstRide().YEAR + ";" + schedule.getFirstRide().HOUR_OF_DAY + ":" + schedule.getFirstRide().MINUTE );
		Paragraph lastDay = new Paragraph("Letzter Fahrtag : " + schedule.getLastRide());
		List<String> drivingDaysArray = new ArrayList<String>();
		for (int i : schedule.getDays()){
			drivingDaysArray.add(getWeekDay(i));
		}
		Paragraph drivingDays = new Paragraph("Fährt an den Tagen : " + drivingDaysArray);
		Paragraph intervall = new Paragraph("Intervall : " + schedule.getInterval());
		Paragraph railsys = new Paragraph("Gehört zu Streckennetznummer : " + schedule.getRailSysID());
	 ...
	 *    
	 */
	protected void addStatisticContent (Document document, Object data, ScheduleScheme schedule) {
		if (document == null) {
			throw new IllegalArgumentException(
					"document must not be null");
		}
	 ...
	 *    
	 */
	protected void addDepartureContent (Document document, Object data, ScheduleScheme schedule) {
		if (document == null) {
			throw new IllegalArgumentException(
					"document must not be null");
		}
