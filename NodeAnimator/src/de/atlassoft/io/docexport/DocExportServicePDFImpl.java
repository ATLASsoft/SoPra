...
/**
 * Implementation of the interface "DocExportService" providing access to the
 * persistence layer.
 * 
...
public class DocExportServicePDFImpl implements DocExportService {
	...
	public void createStatisticDoc(SimulationStatistic stat)
		...
		PdfWriter.getInstance(document,
				new FileOutputStream(STATISTIC_DOC_PATH.toString() + ".pdf"));
		document.open();
		...
		pdfCreator
				.addTitlePage(
						...
						"Dieses Dokument, beinhaltet alle relevanten Informationen zu der Statistik des letzten Simulationdurchlaufs",
						"");
		pdfCreator.addStatisticContent(document, stat);
		document.close();
		try {
			Runtime.getRuntime().exec(
					"cmd.exe /c start " + STATISTIC_DOC_PATH.toString()
							+ ".pdf");
		} catch (Exception e) {
	...
	public void createScheduleDoc(ScheduleScheme schedule)
		...
		pdfCreator
				.addTitlePage(
						...
						"Dieses Dokument, beinhaltet alle relevanten Informationen zu dem Fahrplan: ",
						schedule.getID());
	...
	@Override
	public void createDepartureBoard(Node station, List<ScheduleScheme> scedList)
			throws DocumentException, MalformedURLException, IOException {
		Document document = new Document();
		PdfWriter.getInstance(document,
		...
		pdfCreator
				.addTitlePage(
						document,
						"Abfahrtentafel",
						"Dieses Dokument, beinhaltet alle relevanten Informationen, zu allen Abfahrtszeiten, zu der Station: ",
						station.getName());
